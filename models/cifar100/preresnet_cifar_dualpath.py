...
# from models.cifar100.shufflenet import ShuffleUnit

__all__ = ['preact_resnet20_cifar100_dualpath', 'preact_resnet44_cifar100_dualpath', 'preact_resnet56_cifar100_dualpath',
            'preact_resnet110_cifar100_dualpath', 'preact_resnet20_cifar100_dualpath_rl', 'preact_resnet44_cifar100_dualpath_rl', 
            'preact_resnet56_cifar100_dualpath_rl', 'preact_resnet110_cifar100_dualpath_rl']
...
class DualpathBlock(nn.Module):
    ...
    def __init__(self, basic_block, gate, fullbit_block, inplanes, outplanes, return_gate_states=False, is_first_block=False, pre_gate=False):
...
class PreactResNetCifarDualpath(nn.Module):
    def __init__(self, block, layers, num_classes=NUM_CLASSES, conv_downsample=False, gate_channel=0, return_gate_states=False, pre_gate=False, **kwargs):
        self.nlayers = 0
        self.return_gate_states = return_gate_states
        self.pre_gate = pre_gate
        ...
        self.layer1 = self._make_layer(self.layer_gates[0], block, 64, layers[0],
                                       conv_downsample=conv_downsample, pool_size=32, gate_channel=gate_channel)
        self.layer2 = self._make_layer(self.layer_gates[1], block, 128, layers[1], stride=2,
                                       conv_downsample=conv_downsample, pool_size=16, gate_channel=gate_channel)
        self.layer3 = self._make_layer(self.layer_gates[2], block, 256, layers[2], stride=2,
                                       conv_downsample=conv_downsample, pool_size=8, gate_channel=gate_channel)
        self.final_bn = nn.BatchNorm2d(256 * block.expansion)
    ...
    def _make_layer(self, layer_gates, block, planes, blocks, stride=1, conv_downsample=False, pool_size=16, gate_channel=0):
        ...
        layers.append(
            block(
                ...
                gate = gate(pool_size=pool_size*2 if self.pre_gate else pool_size,
                             channel=self.inplanes if self.pre_gate else planes*block.expansion, 
                             gate_channel=planes*block.expansion if gate_channel==0 else gate_channel),
